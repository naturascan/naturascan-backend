...
class ExportsDataExport implements FromCollection, WithHeadings, WithMapping
{
    ...
    function formatGPS(array $GPSData): string
    {
        // Vérifie si la clé 'gps' existe et si elle est non vide
        if(is_null($GPSData)){
                return '';
            }

        // Initialisation de la structure GeoJSON
        $geojson = [
            "type" => "FeatureCollection",
            "features" => []
        ];

        // Boucle sur les points GPS
        foreach ($GPSData as $point) {
            // S'assurer que les coordonnées existent avant de continuer
            if (!isset($point['longitude']) || !isset($point['latitude'])) {
                continue; // Passer au point suivant s'il manque des coordonnées
            }

            $longitude = (float)$point['longitude'];
            $latitude = (float)$point['latitude'];

            // Préparation des propriétés : retirer les coordonnées pour qu'elles n'apparaissent que dans 'geometry'
            $properties = $point;
            unset($properties['longitude']);
            unset($properties['latitude']);

            // Construction de l'objet Feature
            $feature = [
                "type" => "Feature",
                "geometry" => [
                    "type" => "Point",
                    // Ordre GeoJSON standard : [Longitude, Latitude]
                    "coordinates" => [$longitude, $latitude]
                ],
                "properties" => $properties
            ];

            // Ajout du Feature à la FeatureCollection
            $geojson['features'][] = $feature;
        }

        // Encodage final en chaîne JSON, formatée pour une meilleure lisibilité
